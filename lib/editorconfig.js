...
module.exports = class Editorconfig {
	...
	// Subscriber handlers
	subscribeHandlers() {
		this.disposables.add(this.buffer.onWillSave(this.onWillSave))
	}
	...
	applySettings() {
		...
		if ('endOfLine' in settings && settings.endOfLine in EOL_CHARS) {
			// Inofficial API in the text-buffer
			buffer.setPreferredLineEnding(EOL_CHARS[settings.endOfLine]);
		}
	}
	...
	// Returns the current TextEditor or undefined if not found
	_getCurrentEditor() {
